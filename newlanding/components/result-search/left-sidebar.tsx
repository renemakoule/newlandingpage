...
import { Home, ChevronDown, Video, Package, Film, Filter, X, Menu, Image, Send } from "lucide-react"
...
export function LeftSidebar({
  ...
  onContentTypeChange: (type: "products" | "videos" | "webSeries" | "images") => void
  onHomeClick: () => void
  activeContentType: "products" | "videos" | "webSeries" | "images"
}) {
  ...
  const { selectedCategory, setSelectedCategory, setCurrentView } = useSearchStore()
  ...
  const handleContentTypeChange = (type: "products" | "videos" | "webSeries" | "images") => {
      ...
      <div className="p-4 space-y-6">
        <div className="space-y-4">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" className="w-full justify-start" onClick={onHomeClick}>
        ...
        <div className="space-y-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant={activeContentType === "products" ? "default" : "outline"}
                className={`w-full justify-start ${activeContentType === "products" ? "bg-primary text-primary-foreground" : ""}`}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className={`w-full justify-start ${activeContentType === "videos" ? "bg-primary text-primary-foreground" : ""}`}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant={activeContentType === "webSeries" ? "default" : "outline"}
                className={`w-full justify-start ${activeContentType === "webSeries" ? "bg-primary text-primary-foreground" : ""}`}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className={`w-full justify-start ${activeContentType === "images" ? "bg-primary text-primary-foreground" : ""}`}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="w-full">
                <Filter className="mr-2 h-5 w-5" /> {sortBy === "name" ? "Sort by Name" : "Sort by Price"}
              </Button>
            </DropdownMenuTrigger>
            ...
            <AnimatePresence>
              {searchHistory.map((item, index) => (
                ...
                >
                  ...
                  <Button variant="ghost" size="icon" onClick={() => removeSearchHistoryItem(item)} className="h-6 w-6">
                    <X className="h-4 w-4" />
                  </Button>
                </motion.li>
              ))}
  ...
  return (
    <TooltipProvider>
      {isMobile && (
        <Button variant="outline" size="icon" className="fixed top-4 left-4 z-50" onClick={toggleMobileMenu}>
          <Menu className="h-6 w-6" />
        </Button>
      )}
      <motion.div
        className={`fixed left-0 top-0 bottom-0 w-64 bg-background border-r border-border ${isMobile ? "z-40" : ""}`}
